  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">등록된 작업 사례 {portfolios.length}개</p>
        <button
          onClick={() => {
            setShowAdd(true)
            setForm(EMPTY_FORM)
      ...
      {showAdd && (
        <div className="rounded-2xl border border-indigo-100 bg-indigo-50/30 p-5 shadow-sm">
          <h3 className="mb-4 font-semibold text-gray-800">새 작업 사례 등록</h3>
      ...
      {portfolios.length === 0 && !showAdd ? (
        <div className="rounded-2xl border border-dashed border-gray-200 py-16 text-center">
          <p className="text-gray-400">등록된 작업 사례가 없습니다.</p>
          <p className="mt-1 text-sm text-gray-300">작업 사례를 등록하면 고객에게 노출됩니다.</p>
        </div>
      ) : (
                ...
                {editId === p.id ? (
                  ...
                  <>
                    <div className="mb-3">
                      ...
                      {p.completed_at && (
                        <p className="mt-1 text-xs text-gray-400">{p.completed_at} 완료</p>
                      )}
                    </div>
                    {/* 삭제 확인 */}
                    {deleteId === p.id ? (
                      <div className="space-y-2 rounded-xl bg-red-50 p-3">
                        <p className="text-xs font-medium text-red-700">삭제하시겠습니까?</p>
                        <div className="flex gap-2">
                          <button
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="flex-1 rounded-lg border border-red-100 py-1.5 text-xs text-red-500 hover:bg-red-50"
                        >
                          삭제
                        </button>
                      </div>
...
function PortfolioFormFields({
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        <label className="mb-1 block text-xs font-medium text-gray-600">완료일</label>
        <input
          type="date"
