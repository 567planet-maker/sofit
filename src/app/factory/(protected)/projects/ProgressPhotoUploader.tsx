  ...
  return (
    <div className="space-y-4">
      ...
      {photos.length > 0 ? (
        ...
        <p className="text-sm text-gray-400">아직 등록된 진행 사진이 없습니다.</p>
      )}

      {/* 새 사진 업로드 */}
      <div className="rounded-xl border border-gray-100 bg-gray-50 p-4">
        <p className="mb-3 text-xs font-medium text-gray-600">진행 사진 추가</p>
        <FileUploader
        ...
        {uploadedFiles.length > 0 && (
          <button
            onClick={handleSave}
            disabled={isPending}
            className="mt-3 w-full rounded-xl bg-indigo-600 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50"
          >
