...
distributing  → 파랑    공장배포중    (관리자가 공장들에게 배포 중)
quote_arrived → 초록    견적서 도착   (수락 공장들의 견적서 도착)
contracted    → 진한초록 계약 완료    (고객이 공장 선택·최종 확정)
in_progress   → 주황    시공중        (고객↔공장 직접 소통·작업 진행)
...
```
pending    → 노랑   배포됨 (공장이 검토 중)
accepted   → 파랑   공장 수락 (견적서 작성 가능)
declined   → 빨강   공장 거절
confirmed  → 진한초록 최종 선택됨 (고객이 이 공장 선택)
cancelled  → 회색   취소
```

### 견적서(factory_quotes) 상태

```
draft      → 회색   작성 중
submitted  → 파랑   제출됨 (고객에게 전달)
accepted   → 초록   채택됨 (고객이 이 견적 선택)
rejected   → 빨강   미채택
```
...
공장 견적서 비교 (/customer/requests/[id]/quotes)
  │
  ├─ 공장 A 카드 (총액·납기·포트폴리오 링크) [이 공장 선택]
  ├─ 공장 B 카드                              [이 공장 선택]
  └─ [소핏에게 문의] → 매칭 전 소핏 채팅방
       │
       ▼ "이 공장 선택" 클릭
관리자 최종 확정 알림 → 상태: "계약 완료"
  │
  ▼
고객↔공장 직접 채팅방 개설
  ├─ 일정·자재·납기 직접 협의
...
요청 상세 (/factory/requests/[id])
  │ 고객 정보·폼 내용·사진 열람
  │
  ├─ [수락하기] 클릭 → matches.status = 'accepted'
  │    │
  │    ▼ 견적서 작성 폼 활성화
  │  견적서 작성
  ...
  │    └─ [고객에게 제출] 클릭
  │         │
  │         ▼ DB INSERT + 고객 알림 (관리자 알림 없음)
  │       "견적서가 고객에게 전달되었습니다" 확인
  │
  └─ [거절하기] 클릭 → matches.status = 'declined'
       "참여하지 않기로 했습니다" 확인
```
...
요청 상세 (/admin/requests/[id])
  ...
  ├─ 상태 → "reviewing"으로 변경
  │
  └─ [공장 배포] 클릭 → /admin/requests/[id]/match
       └─ 상태 → "distributing"
```
...
```
공장 배포 (/admin/requests/[id]/match)
  │
  ├─ 활성 공장 목록 (위치·포트폴리오 필터)
  │
  ├─ 공장 A 카드 [배포] 클릭
  │    └─ matches INSERT (status='pending') + 공장 알림
  │
  ├─ 배포된 공장 목록 + 수락/거절 현황 표시
  │    ├─ 천일쇼파 [수락] — 견적서 작성중
  │    └─ 부산쇼파 [검토중]
  │
  └─ 고객이 공장 선택 후 → [매칭 확정] 버튼
       └─ matches.status = 'confirmed' + 고객↔공장 채팅방 생성
```
...
채팅 통합 뷰 (/admin/chats)
  │
  ├─ 왼쪽: 전체 채팅방 목록
  │    ├─ [매칭 전] 고객↔소핏 채팅 (미답변 우선)
  │    └─ [매칭 후] 고객↔공장 직접 채팅 (모니터링)
  │
  └─ 오른쪽: 선택된 채팅방 메시지
       ├─ 고객↔소핏 채팅: 고객 질문 확인 → 답변 (양방향)
       └─ 고객↔공장 채팅: 읽기 전용 모니터링
            └─ 분쟁 시 해당 채팅 기록 증거로 활용
```
