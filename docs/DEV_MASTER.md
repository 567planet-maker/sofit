...
- [x] **공장 자율 매칭 참여** (`joinRequest` 서버 액션 + `JoinActions.tsx`)
  - 활성 공장이 요청 목록에서 신규 요청을 직접 확인 → "매칭 참여하기" 클릭으로 즉시 confirmed 전환
  - 참여 즉시 `customer_factory` 채팅방 자동 생성 + 고객 알림
- [x] **고객 견적서 수락** (`acceptFactoryQuote` 서버 액션 + `QuoteVersionCard.tsx`)
  - "이 견적서로 계약하기" → 2단계 확인 → `factory_quotes.status = accepted`, `quote_requests.status = contracted`
  - 공장에 수락 알림 + `status_logs` 기록
- [x] **견적서 버전 비교** (`QuoteVersionCard.tsx` — 항목별 diff 테이블)
  - 수정 견적서(v2+) 도착 시 이전 버전과 항목별 변동금액 표시
- [x] **공장 요청 목록 개선** (`factory/requests/page.tsx`)
  - 신규 요청(미참여) / 내 참여 요청 분리 표시 — 활성 공장은 모든 신규 요청 열람 가능
- [x] **고객 채팅 목록 개선** (`customer/chat/page.tsx`)
  - `customer_factory` 방에 공장명 표시 ("천일쇼파 채팅" 등)
- [x] **마이그레이션 004~007** (Supabase SQL 에디터 적용 완료)
  - 004: `factory_quotes` 버전 관리 — `superseded` 상태 + `is_latest` 컬럼 + 성능 인덱스
  - 005: 고객 견적수락 RLS + 공장 상태변경 RLS
  - 006: 무한 재귀 유발 정책 제거 (service client 방식으로 전환)
  - 007: 공장 신규 요청 전체 조회 정책 + 자율 참여 INSERT 정책

### 기능 테스트 (브라우저)

- [ ] **고객 E2E**: 랜딩 → 로그인 → 폼 제출 → 상태 확인 → 견적서 수신 → 수락 → 채팅
- [ ] **공장 E2E**: 가입 → 심사 → 포트폴리오 → 신규 요청 확인 → 자율 참여 → 견적서 제출
- [ ] **관리자 E2E**: 요청 수신 → 공장 배포 → 매칭 현황 확인 → 채팅 모니터링
...
- [x] 서버 액션 권한 패턴: 인증은 user client → DB 쓰기는 service client (RLS 우회, 이중 검증)
...
- [ ] 테스트 견적 요청 1건 → 공장 자율 참여 → 견적서 제출 → 고객 수락 전 과정 확인
